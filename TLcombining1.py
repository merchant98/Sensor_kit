...
GPIO.setup(26, GPIO.IN)  # touch s/w as an input pin 26  
root = Tk()
...
def atmp():
    ...
    btn_q.place(x=150, y=325)
    btn_g=Button(wap, text="Plot", command = graph)
    btn_g.place(x=200, y=375)
...
def rtd():
    ...
    def rtdupdate():
        global name1_file
        Time = '{:%d/%m/%y %H:%M:%S}'.format(datetime.datetime.now())
        temp_output = (analogInput(3)/1023*100)+0.4 #Channel Number
        #temp_output = round(temp_output, 2)
        output_ldr.set("%d%%" %temp_output)
        print("%d" %temp_output)
        with open(name1_file, 'a') as csv_file:
            ...
            csv_writer.writerow(info)
        global id1
        id1=winrtd.after(3000,rtdupdate)
    ...
    def rtdshow():
        ...
        with open(name1_file) as f:
            ...
            for row in reader:
                ...
                Temperature = row['Temperature']
                #Humidity = row['Humidity']
                tree.insert("", 0, values=(Time, Temperature))

        #============================INITIALIZATION==============================
        if __name__ == '__main__':
            tab.mainloop()
            
    output_rtd = tk.StringVar()    
    winrtd = Toplevel()
    winrtd.geometry("500x500")
    winrtd.configure(background='#CD5C5C')
    winrtd.title('LDR Hai Apun') 
    label_1_rtd = Label(winrtd, text='Light: ', font=("Helvetica", 15), height=2, width=6, bg='#CD5C5C')
    label_1_rtd.place(x=120, y=92)
    output_1_label_rtd = Label(winrtd, textvariable=output_ldr, height=2, width=12)
    output_1_label_rtd.place(x=200, y=100)
    btn_rtdbegin=Button(winrtd, text="Begin", command = rtdupdate)
    btn_rtdbegin.place(x=150, y=275)
    btn_rtddestroy=Button(winrtd, text="Quit", command = winrtd.destroy)
    btn_rtddestroy.place(x=250, y=275)
    btn_rtdshow=Button(winrtd, text="Show", command = rtdshow)
    btn_rtdshow.place(x=150, y=325)
    btn_rtdpause=Button(winrtd, text="Pause", command = rtdpause)
    btn_rtdpause.place(x=250, y=325)
    winrtd.mainloop()
